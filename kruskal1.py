    ...
    def visualize2(self,res): #for the result 
        ...
        for i in range(n):
            ...
            if(res[i][0] not in temp and res[i][1] not in temp):
                ...
                temp.append(res[i][1])
        m=len(temp)
        fixed_positions = {0:(-1,-3),1:(1,0),2:(-1,2),3:(1,-3)}
        for i in range(m):
...
def kruskal1():
    ...
    G=nx.Graph()
  #[2, 3, 4], [0, 3, 5], [0, 1, 10]
    G.add_edges_from([(2,3),(0,3),(0,1)]) 
    p=nx.Graph()
    p.add_edges_from([(0,1),(0,2),(0,3),(1,3),(2,3)])
    ...
    imageio.mimsave("C:\\Users\\HRIDHI SETHI\\Documents\\openlab\\static\\images\\slowgif1.gif",gif,fps=1)
    return 